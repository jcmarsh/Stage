...
long playerc_simulation_get_time(playerc_simulation_t *device, long x) {
  player_simulation_time_req_t req, *resp;

  memset(&req, 0, sizeof(req));

  if (playerc_client_request(device->info.client, &device->info,
			     PLAYER_SIMULATION_REQ_GET_TIME, &req,
			     (void*)&resp) < 0) {
    printf("File: %s:%d\t Ther is an error in simulation_get_time.\n", __FILE__, __LINE__);
    return (-1);
  }
